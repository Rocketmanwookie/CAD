...
10. Configure a catalog PLC CPU and DI/DO/AI/AO counts, then run **Allocate PLC I/O**. Confirm that the I/O list receives a complete rack/slot/channel mapping without capacity or collision findings and that persistent PLC rack/module/channel occurrences appear. Each allocated channel has a deterministic typed-signal link; paths created later for that tag reuse the same signal and appear in the channel's path links. The path-authoring form still starts from a generic registered PLC controller rather than a selected allocated-channel terminal, so complete endpoint-owner authoring and the associated manual save/reopen check remain open work.
